						...
						<tbody>
							{order.cart.map((item, itemIndex) => (
								<tr key={itemIndex}>
									...
									<td>
										<Link to={`/order-status/${order.orderId}`}>View</Link>
									</td>
								</tr>
							))}
						</tbody>
		...
		return (
			<section id='pagination' className='section-p1'>
				...
				{Array.from({ length: endPage - startPage + 1 }, (_, index) => (
					<button key={startPage + index} onClick={() => setCurrentPage(startPage + index)} className={currentPage === startPage + index ? 'active' : ''}>
						{startPage + index}
					</button>
				))}
