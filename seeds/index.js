...
for (let i = 0; i < shoppings.length; i++) {
	...
	const shopping = new Shopping(item.orderList, item.cart, custIds[i]);
...
for (let item of products) {
	const product = new Product(item.name, item.description, item.imgURLs, item.price * 10000, item.rate, 0, null, null);
